...
`src/core/types.ts` contains the TypeScript representation of all JSON-RPC objects:
...
`src/core/protocol.ts` — `detectVersion(raw: unknown): 1 | 2`
  ...
  src/
    ...
    adapters/
      ...
      nestjs/
        ...
        decorator.ts
        service.ts
    index.ts           ← core barrel re-export
...
**Build tool:** `tsup`
- Entry points: `src/index.ts`, `src/transports/http/index.ts`, `src/transports/tcp/index.ts`, `src/transports/ws/index.ts`, `src/adapters/express.ts`, `src/adapters/fastify.ts`, `src/adapters/nestjs/index.ts`
- Output formats: `esm`, `cjs`
- `.d.ts` files generated for all entries
- `package.json` exports map points to `dist/` outputs

**Test tool:** `vitest` with `@vitest/coverage-v8`
...
| Single package, subpath exports | Simpler versioning vs monorepo; tree-shaking means users only pay for what they import |
